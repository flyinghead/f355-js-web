const LOOKUP_TABLE = [ 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7, 0x8108, 0x9129,
...
export default function crc16(bytes: Buffer): number
{
    let crc = 0;

    for (let n of bytes)
        crc = ((crc << 8) ^ LOOKUP_TABLE[((crc >>> 8) ^ n) & 0xFF]);
